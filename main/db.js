      ...
      if (err) {
        ...
        db.run('PRAGMA foreign_keys = ON;', (pragmaErr) => {
          if (pragmaErr) {
            ...
            anaBirimKisaAd TEXT,
            cevrimKatsayisi REAL NOT NULL DEFAULT 1   -- YENİ SÜTUN
          )`, (err) => {
                if (err) {
                  console.error('birimler tablosu oluşturma hatası:', err.message);
                } else {
                  console.log('birimler tablosu hazır.');
                  // Varsayılan birimleri ekleme fonksiyonu, cevrimKatsayisi'ni da içerecek şekilde güncellenmeli
